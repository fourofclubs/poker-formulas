...
object Gen {
  ...
  def choose(start: Int, stopExclusive: Int): Gen[Int] = Gen(nonNegativeLessThan(stopExclusive - start).map { _ + start + 1 })
  def unit[A](a: => A): Gen[A] = Gen[A](State.unit(a))
  def boolean: Gen[Boolean] = Gen[Boolean](nonNegativeLessThan(2).map { _ == 1 })
